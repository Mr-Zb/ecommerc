package com.fun.framework.utils.support.ip2region;
...
public class DbSearcher {
	...
	public DbSearcher(DbConfig dbConfig, File dbFile) throws FileNotFoundException {
		this.dbConfig = dbConfig;
		raf = new RandomAccessFile(dbFile, "r");
	}
	...
	/**
	 * get the region throught the ip address with memory binary search
	 * algorithm
	 * 
	 * @param ip
	 * @return DataBlock
	...
	public DataBlock btreeSearch(long ip) throws IOException {
		...
		byte[] iBuffer = new byte[blockLen + blen]; // include the right border
													// block
		raf.seek(sptr);
